...
//  Nexvent
//
//  Created by Anthony McMillan on 13/05/2021.
//
...
class Chart: UIView {
    ...
    private func setupView() {
        ...
        gradientLayer.colors =  [UIColor(red: (86/255.0), green: (204/255.0), blue: (242/255.0), alpha: 1).cgColor , UIColor(red: (47/255.0), green: (128/255.0), blue: (237/255.0), alpha: 0.1).cgColor]
    ...
    override func layoutSubviews() {
        ...
        if let dataEntries = dataEntries {
            scrollView.contentSize = CGSize(width: CGFloat(dataEntries.count) * lineGap - 60, height: 100)
            mainLayer.frame = CGRect(x: 0, y: 0, width: CGFloat(dataEntries.count) * lineGap, height: 100)
            dataLayer.frame = CGRect(x: 0, y: topSpace, width: mainLayer.frame.width, height: mainLayer.frame.height - topSpace - bottomSpace)
            gradientLayer.frame = CGRect(x: 0, y: topSpace, width: mainLayer.frame.width, height: self.frame.size.height)
    ...
    private func drawCurvedChart() {
        ...
        if let path = CurveAlgorithm.shared.createCurvedPath(dataPoints) {
            let lineLayer = CAShapeLayer()
            lineLayer.path = path.cgPath
            lineLayer.strokeColor = UIColor(red: (86/255.0), green: (204/255.0), blue: (242/255.0), alpha: 1).cgColor
    ...
    private func maskGradientLayer() {
        if let dataPoints = dataPoints,
            ...
            let path = UIBezierPath()
            path.move(to: CGPoint(x: dataPoints[0].x, y: self.frame.size.height))
            ...
            path.addLine(to: CGPoint(x: dataPoints[dataPoints.count-1].x, y: self.frame.size.height))
            path.addLine(to: CGPoint(x: dataPoints[0].x, y: self.frame.size.height))
            
            let maskLayer = CAShapeLayer()
            maskLayer.path = path.cgPath
            maskLayer.fillColor = UIColor(red: (0/255.0), green: (114/255.0), blue: (255/255.0), alpha: 1).cgColor
    ...
    private func drawLables() {
        if let dataEntries = dataEntries,
            ...
            for i in 0..<dataEntries.count {
                ...
                textLayer.frame = CGRect(x: lineGap*CGFloat(i) - lineGap/2 + 40.5, y: mainLayer.frame.size.height - bottomSpace/2 - 8, width: lineGap, height: 16)
    ...
    private func drawHorizontalLines() {
        ...
        if let gridValues = gridValues {
            for value in gridValues {
                let height = value * gridLayer.frame.size.height - 15
